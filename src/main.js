...
function hud(){const p=world.player,w=WEAPONS[p.weapon],sector=String(world.level.index+1).padStart(2,'0');$('health').textContent=Math.ceil(p.health);$('shield').textContent=Math.ceil(p.shield);$('health-bar').style.width=p.health+'%';$('shield-bar').style.width=p.shield+'%';$('health-bar').style.background=p.health<30?'#ff9a7f':'#8df8ce';$('weapon-name').textContent='0'+(p.weapon+1)+' / '+w.name;$('ammo').textContent=w.ammo?p[w.ammo]:'∞';$('blue-key').classList.toggle('active',p.keys.blue);$('red-key').classList.toggle('active',p.keys.red);$('relays').textContent='RELAYS '+world.relayCount+'/2';$('level-number').textContent=sector;$('level-name').textContent=world.level.config.name;$('sector').textContent='SECTOR '+sector+' / '+world.level.config.name;$('objective').textContent=world.objective();$('notice').textContent=world.messageTime>0?world.message:'';$('prompt').textContent=playing?world.prompt():'';}
function debrief(){
