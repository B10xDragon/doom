...
export function buildLevel(index=0) {
  ...
  if(index===2){for(const [x,y] of [[3,14],[5,14],[7,17],[8,17],[17,4],[17,5],[17,6],[23,15],[24,15],[26,15]])grid[y][x]=3;}
  ...
  const enemy=(kind,x,y)=>enemies.push({kind,x:x+.5,y:y+.5,health:kind==='warden'?360:kind==='sentinel'?78:42,cooldown:1.5,alert:false,hurt:0,dead:false});
  ...
  if(index>0){enemy('sentinel',8,24);enemy('drone',20,18);enemy('sentinel',27,26);}
  if(index===2)enemy('warden',26,18);
